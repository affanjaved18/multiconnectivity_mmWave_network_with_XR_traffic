...
if __name__ == "__main__":
	...
	list_of_schedulers = ["EDF","RR","PF","BESTCQI","MWM"]
	list_of_predictors = ["RANDOM","BESTCQI"]
	...
	filename = f'{scheduler}_{beta}_{predictor}_{N}_{M}_{K}_{L}_{env.connectivity_handover_delay}_{env.replication_handover_delay}.csv'
	filepath ="/home/maj407/Other Schedulers/"
	...
	T = int(T/env.scheduling_interval) 
	...
	for i in range(T):
		...
		if scheduler.strip().lower() == 'edf':
			...
			action = env.MWM_Heuristic(beta)

		cp2 = time.time()
		# print("Scheduling Time: ", cp2 - cp1)

		env.stats_total_service_time += action
		
		reward = env.traffic_transitions(action)    #equivalent to taking a step
		cp3 = time.time()
		# print("Traffic Transitions Time: ", cp3 - cp2)

	    #update channel states for the next time slot
		env.channel_transitions(predictor)
		cp4 = time.time()
		# print("Channel Transitions Time: ", cp4 - cp3)
		# print("")
		...
		for k in range(env.UEs):
			...
			if env.stats_outage_counter[k] == 0 and np.sum(current_UE.BS_Replication_Set) == 0:
				...
				# print("UE: ",k)
				# print("Time: ",i)
				# print("Replication Set: ",current_UE.BS_Replication_Set)
		...
		# print("One time-slot execution time: ",cp5-cp0)
	...
	for i in range(env.UEs):
		...
		count_served=0
		count_expired=0
		# print("Time Spent in Buffer: ")
		# print(current_UE.frames_time_spent_in_buffer)
		# print("Arrival Times: ")
		# print(current_UE.stats_arrival_times)
		# print("Service/Expiry Times: ")
		# print(current_UE.time_served_or_expired)
	...
	# output2 = env.stats_system_total_num_blockages
	# filename2 = f'numOutages_{scheduler}_{predictor}_{N}_{M}_{K}_{L}_{env.connectivity_handover_delay}_{env.replication_handover_delay}.csv'
	# output_file2 = filepath+filename2
	# with open(output_file2, "a") as f2:
	# 	np.savetxt(f2,output2,delimiter=',')
